...
impl Ticket {
    fn is_empty(string: &String, label: &str) {
        if string.is_empty() {
            panic!("{} cannot be empty", label)
        }
    }
    fn len(string: &String, len: usize, label: &str) {
        if string.len() > len {
            panic!("{} cannot be longer than {} bytes", label, len)
        }
    }
    ...
    pub fn new(title: String, description: String, status: String) -> Ticket {
        Ticket::is_empty(&title, "Title");
        Ticket::len(&title, 50, "Title");
        Ticket::is_empty(&description, "Description");
        Ticket::len(&description, 500, "Description");
